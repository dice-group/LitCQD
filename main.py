...
def create_latex_table(results, model_name):
    ...
    document = template.render(model_name=model_name, query_structures=query_structures, metrics=values_by_metric)
    print(document)
...
def evaluate(tester, tp_answers, fn_answers, train_config: TrainConfig, query_name_dict, mode, epoch):
    '''
    ...
    for metric in average_metrics:
        ...
        if writer:
            writer.add_scalar("_".join([mode, 'average', metric]), average_metrics[metric], epoch)
        all_metrics["_".join(["average", metric])] = average_metrics[metric]
    for metric in average_metrics_attr:
        average_metrics_attr[metric] /= num_query_structures_attr
        if writer:
            writer.add_scalar("_".join([mode, 'average', metric]), average_metrics_attr[metric], epoch)
...
def test_model(model, train_config: TrainConfig, mode='Test', tasks=('1p', '1ap',)):
    ...
    queries, easy_answers, hard_answers = load_queries_eval(train_config.data_path, tasks, mode.lower())
...
def run_tune(train_config: TrainConfig, cqd_params: CQDParams, params: HyperParams, nentity, nrelation, nattribute,
    ...
    if train_config.geo.name in ('cqd-complexd', 'cqd-complexad'):
        ...
        print("Best trial final valid loss: {}".format(best_trial.last_result["valid_loss"]))
        print("Best trial final validation mrr: {}".format(best_trial.last_result["mrr"]))
        print("Best trial final validation cos_sim: {}".format(best_trial.last_result["cos_sim"]))
        print("Best trial final train cos_sim: {}".format(best_trial.last_result["train_cos_sim"]))
        print("Best trial final validation di MRR: {}".format(best_trial.last_result["di_mrr"]))

        best_params = HyperParams(**best_trial.config)
        best_trained_model = get_model(train_config, best_params, cqd_params, nentity, nrelation, nattribute)
        ...
        print("Best trial (loss) final validation loss: {}".format(best_trial.last_result["loss"]))
        print("Best trial (loss) final validation attr_loss: {}".format(best_trial.last_result["attribute_loss"]))
        print("Best trial (loss) final validation rel_loss: {}".format(best_trial.last_result["rel_loss"]))
        print("Best trial (loss) final validation score: {}".format(best_trial.last_result["score"]))
        print("Best trial (loss) final validation mrr: {}".format(best_trial.last_result["mrr"]))
        print("Best trial (loss) final validation mae: {}".format(best_trial.last_result["mae"]))
        ...
        print("Best trial (mrr) final validation loss: {}".format(best_trial.last_result["loss"]))
        print("Best trial (mrr) final validation attr_loss: {}".format(best_trial.last_result["attribute_loss"]))
        print("Best trial (mrr) final validation rel_loss: {}".format(best_trial.last_result["rel_loss"]))
        print("Best trial (mrr) final validation score: {}".format(best_trial.last_result["score"]))
        print("Best trial (mrr) final validation mrr: {}".format(best_trial.last_result["mrr"]))
        print("Best trial (mrr) final validation mae: {}".format(best_trial.last_result["mae"]))
...
def main(args):
    ...
    if args.train_config.save_path is None:
        ...
        if args.train_config.checkpoint_path is not None:
            ...
            args.train_config.save_path = os.path.join(args.train_config.save_path, parse_time())
    ...
    if args.train_config.do_tune:
        writer = None
    elif not args.train_config.do_train:  # if not training, then create tensorboard files in some tmp location
