...
from models.util import flatten_structure, query_to_atoms, symbol_placeholder_dict, name_query_dict, query_name_dict
...
class CQDBaseModel(nn.Module):
    ...
    def predict_attribute_values_all(self, attributes):
        ...
        for i in range(unique_attr.shape[0]):
            ...
            values[idxs] = self.predict_attribute_values(self.ent_embeddings.weight, unique_attr[i].expand(self.nentity)).expand(idxs.shape[0], -1)
        return values
    ...
    def score_attribute_exists(self, attribute):
        ...
        """
        attr_exists_ent = self.ent_embeddings(torch.tensor([self.nentity-1]).to(device=attribute.device))

        a_emb = self.rel_embeddings(2*attribute+self.nrelation-2*self.nattr)  # *2 to skip inverse relations
        all_scores = self.score_o_all(attr_exists_ent.expand(1, 1, -1), a_emb.expand(1, 1, -1), self.ent_embeddings.weight).squeeze()
        ...
        def get_score(e_emb=None):
            ...
            if e_emb is not None:
                ...
                e_scores = self.score_o(attr_exists_ent.expand(e_emb.shape[0], -1), a_emb.expand(e_emb.shape[0], -1), e_emb)
            ...
            if e_emb is not None:
                ...
                scores = torch.cat((all_scores_tmp, torch.FloatTensor([.0]).to(device=attribute.device)), dim=-1)
            return scores
    ...
    def score_attribute_restriction(self, filters: Tensor, attributes: Tensor):
        ...
        get_attr_exists_scores = [self.score_attribute_exists(attr) for attr in unique_attr]
        ...
        def attr_exists_ids_within_stdev(x):
            ...
            """
            res = (x >= torch.max(x)-torch.std(x, dim=-1, unbiased=False)).nonzero(as_tuple=True)[0]
            if len(res) < 10:
        ...
        def _score_attribute_restriction(e_emb=None):
            if e_emb is not None:
                scores = torch.empty((e_emb.shape[0],), requires_grad=True).to(device=e_emb.device)
                predictions = self.predict_attribute_values(e_emb, attributes).unsqueeze(-1)
            else:
                scores = torch.empty_like(all_predictions)
                predictions = all_predictions

            for attr_idx, attr in enumerate(unique_attr):
                idxs = (attributes == attr).nonzero().squeeze(1)  # positions of attr in the batch
                ...
                with torch.no_grad():
                    if e_emb is None:
                        ...
                        attr_exists_scores = attr_exists_scores.expand(idxs.shape[0], -1)
                    else:
    ...
    def normalize_batch(self, tensor):
        ...
        for i in range(tensor.shape[0]):
            result[i] = (tensor[i] - tensor[i].min()) / (tensor[i].max() - tensor[i].min())
        return result
    ...
    def reduce_query_score(self, atom_scores, conjunction_mask):
        ...
        disjunctions = scores[~conjunction_mask].reshape(batch_size, -1, *extra_dims)  # A or B (union)
        conjunctions = scores[conjunction_mask].reshape(batch_size, -1, *extra_dims)  # A and B (intersection)
    ...
    def continuous_loop(self, num_variables, batch_size, atoms, num_var, attr_mask, filters, h_emb_constants, head_vars_mask, conjunction_mask):
        ...
        while i < 1000 and (loss_value > -0.2 or math.fabs(prev_loss_value - loss_value) > 1e-4):
    ...
    def forward(self, batch_queries_dict: Dict[Tuple, Tensor]):
        ...
        for query_structure, queries in batch_queries_dict.items():
            ...
            if 'continuous' in self.method:
                ...
                atoms, num_variables, conjunction_mask, attr_mask, filters, no_head_entity = query_to_atoms(query_structure, queries)
                ...
                attr_prediction = attr_count > filter_count  # the result is an attribute value
                ...
                if num_variables > 1:
                    h_emb = self.continuous_loop(num_variables, batch_size, atoms, num_var, attr_mask, filters, h_emb_constants, head_vars_mask, conjunction_mask)

                with torch.no_grad():
                    # Compute likelihood for each entity in the dataset given h_emb from the CO optimization loop or the head of the query
                    scores = []
                    if len(attr_mask[target_mask].nonzero()) != len(attr_mask[target_mask]):
                        # using reshape to keep dimensions
                        r_emb = self.rel_embeddings(rel[target_mask & ~attr_mask].reshape(rel.shape[0], -1))
                        h_emb = h_emb[target_mask & ~attr_mask].reshape(h_emb.shape[0], -1, h_emb.shape[-1])
                        score = self.score_o_all(h_emb, r_emb, self.ent_embeddings.weight)
                        scores.append(score)

                    # target_mask is equal for each element at dim1 (since query_structure is fixed)
                    filters = [f for i, f in enumerate(filters) if i in target_mask[0].nonzero() and f is not None]
                    ...
                    if len(attr_mask[target_mask].nonzero()) > 0:
                        ...
                        if no_head_entity or len(scores) > 0:
                            ...
                            if filters:
                                predictions = self.score_attribute_restriction(torch.cat(filters).to(device=attributes.device), attributes)(None)
                                predictions = predictions.reshape(batch_size, -1, self.nentity)
                            else:
                                predictions = self.predict_attribute_values_all(attributes)
                        else:
                            # e.g. 1ap, 2ap
                            e = h_emb[target_mask & attr_mask]
                            if filters:
                                predictions = self.score_attribute_restriction(torch.cat(filters).to(device=attributes.device), attributes)(e)
                                predictions = predictions.reshape(batch_size, -1, self.nentity)
                            else:
                                predictions = self.predict_attribute_values(e, attributes)

                        scores.append(predictions)

                    if not attr_prediction or filters:
                        # intersection / union
                        conjunction_mask = conjunction_mask[target_mask].reshape(conjunction_mask.shape[0], -1)
                ...
                if graph_type == "1p":
                    score = d2.query_1p(entity_embeddings=self.ent_embeddings,
                                        predicate_embeddings=self.rel_embeddings,
                                        queries=queries.long(),
                                        scoring_function=scoring_function)
                elif graph_type == "1ap":
                    score = d2.query_1ap(entity_embeddings=self.ent_embeddings,
                                         queries=queries.long(),
                                         val_prediction_function=self.predict_attribute_values)
                elif graph_type == "1dp":
                    if hasattr(self, 'desc_jointly') and self.desc_jointly:
                        score = d2.query_1dp_jointly(entity_embeddings=self.ent_embeddings,
                                                     queries=queries.long(),
                                                     predict_descriptions_function=self.predict_descriptions,
                                                     )
                    else:
                        score = d2.query_1dp(entity_embeddings=self.ent_embeddings,
                                             queries=queries.long(),
                                             predict_descriptions_function=self.predict_descriptions,
                                             scoring_function=scoring_function,
                                             )
                elif graph_type == "2p":
                    score = d2.query_2p(entity_embeddings=self.ent_embeddings,
                                        predicate_embeddings=self.rel_embeddings,
                                        queries=queries.long(),
                                        scoring_function=scoring_function,
                                        k=self.k, t_norm=t_norm)
                elif graph_type == "2ap":
                    score = d2.query_2ap(entity_embeddings=self.ent_embeddings,
                                         predicate_embeddings=self.rel_embeddings,
                                         queries=queries.long(),
                                         scoring_function=scoring_function,
                                         val_prediction_function=self.predict_attribute_values,
                    ...
                    score = d2.query_3p(entity_embeddings=self.ent_embeddings,
                                        predicate_embeddings=self.rel_embeddings,
                                        queries=queries.long(),
                                        scoring_function=scoring_function,
                                        k=self.k, t_norm=t_norm)
                elif graph_type == "3ap":
                    score = d2.query_3ap(entity_embeddings=self.ent_embeddings,
                                         predicate_embeddings=self.rel_embeddings,
                                         queries=queries.long(),
                                         scoring_function=scoring_function,
                                         val_prediction_function=self.predict_attribute_values,
                    ...
                    score = d2.query_2i(entity_embeddings=self.ent_embeddings,
                                        predicate_embeddings=self.rel_embeddings,
                                        queries=queries.long(),
                                        scoring_function=scoring_function, t_norm=t_norm)
                elif graph_type == "3i":
                    score = d2.query_3i(entity_embeddings=self.ent_embeddings,
                                        predicate_embeddings=self.rel_embeddings,
                                        queries=queries.long(),
                                        scoring_function=scoring_function, t_norm=t_norm)
                elif graph_type == "pi":
                    score = d2.query_pi(entity_embeddings=self.ent_embeddings,
                                        predicate_embeddings=self.rel_embeddings,
                                        queries=queries.long(),
                                        scoring_function=scoring_function,
                                        k=self.k, t_norm=t_norm)
                elif graph_type == "ip":
                    score = d2.query_ip(entity_embeddings=self.ent_embeddings,
                                        predicate_embeddings=self.rel_embeddings,
                                        queries=queries.long(),
                                        scoring_function=scoring_function,
                                        k=self.k, t_norm=t_norm)
                    ...
                    if hasattr(self, 'desc_jointly') and self.desc_jointly:
                        score = d2.query_di_jointly(entity_embeddings=self.ent_embeddings,
                                                    queries=queries.long(),
                                                    predict_descriptions_function=self.predict_descriptions,
                                                    scoring_function=scoring_function,
                    ...
                    score = d2.query_ai(entity_embeddings=self.ent_embeddings,
                    ...
                    score = d2.query_2ai(entity_embeddings=self.ent_embeddings,
                                         attribute_embeddings=self.attr_embeddings,
                                         bias_embeddings=self.b,
                                         queries=queries,
                                         score_attribute_restriction=self.score_attribute_restriction,
                    ...
                    score = d2.query_pai(entity_embeddings=self.ent_embeddings,
                                         predicate_embeddings=self.rel_embeddings,
                                         attribute_embeddings=self.attr_embeddings,
                                         bias_embeddings=self.b,
                                         queries=queries,
                    ...
                    score = d2.query_aip(entity_embeddings=self.ent_embeddings,
                                         predicate_embeddings=self.rel_embeddings,
                                         attribute_embeddings=self.attr_embeddings,
                                         bias_embeddings=self.b,
                                         queries=queries,
                    ...
                    score = d2.query_2u_dnf(entity_embeddings=self.ent_embeddings,
                                            predicate_embeddings=self.rel_embeddings,
                                            queries=queries.long(),
                                            scoring_function=scoring_function,
                                            t_conorm=t_conorm)
                elif graph_type == "up":
                    score = d2.query_up_dnf(entity_embeddings=self.ent_embeddings,
                                            predicate_embeddings=self.rel_embeddings,
                                            queries=queries.long(),
                                            scoring_function=scoring_function,
                                            k=self.k, t_norm=t_norm, t_conorm=t_conorm)
                elif graph_type == "au":
                    score = d2.query_au(entity_embeddings=self.ent_embeddings,
                                        attribute_embeddings=self.attr_embeddings,
                                        bias_embeddings=self.b,
                                        queries=queries,
                                        score_attribute_restriction=self.score_attribute_restriction,
